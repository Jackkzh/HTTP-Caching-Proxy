...
void Server::run() {
    ...
    while (true) {
        ...
        cout << "before new thread, client's fd: " << client->client_connection_fd << endl;
        pthread_create(&thread, NULL, handleRequest, client);
    }
}

/**
 * get the request from the client with passed in server side socket fd, receive the request,
 * and store into a outer char * buffer
